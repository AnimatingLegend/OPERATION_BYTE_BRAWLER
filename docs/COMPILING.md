# COMPILING RPG
This document will guide you step-by-step on how to compile your Opus-MP3 for local development.
...
Your terminal needs to know exactly where you game files are located. Use the `cd` (change directory) command to point to your games folder.

**Example**:
```bash
cd C:\Users\YOURNAME\Documents\RPG
```
...
`@eslint/js`, `@types/react`, `@types/react-doms`, `@vitejs/plugin-react`, `eslint`, `eslint-plugin-react-hooks`, `eslint-plugin-react-refresh`, `globals`, `vite`
